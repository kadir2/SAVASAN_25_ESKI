...
class CheckUAV:
    def __init__(self):
        self.rh = redis.Redis(host='localhost', port=6379, db=0)
        self.telemetry_data = []
        self.vehicle = connect('127.0.0.1:14561')
    ...
    #     telenetry_json = self.rh.get('telemetry_data')
    #     if telenetry_json:
    #         try:
    #             telemetry_str = telenetry_json.decode('utf-8')
    #             self.uav_data = json.loads(telemetry_str)
    ...
    def monitor_uav(self, team):
        ...
        while True:
            telemetry_raw = self.rh.get('process_data')
            
            print(f"gelen telemetry_raw: {telemetry_raw}")
            if telemetry_raw:
                try:
                    telemetry_str = telemetry_raw
                    current_telemetry_data = json.loads(telemetry_str)
                    current_telemetry_data = current_telemetry_data.get('konumBilgileri').get('konumBilgileri', [])
                    print(f"aboo: {current_telemetry_data}")
                except (UnicodeDecodeError, json.JSONDecodeError) as e:
                    print(f"Error decoding telemetry data: {e}")
    ...
    def calculate_score(self):
        while True:
            try:
                ...
                if telemetry_raw:
                    try:
                        current_telemetry_data = json.loads(telemetry_raw)
                        current_telemetry_data = current_telemetry_data.get('konumBilgileri').get('konumBilgileri', [])
                    except (UnicodeDecodeError, json.JSONDecodeError) as e:
                ...
                for uav in current_telemetry_data:
                    ...
                    try:
                        ...
                        print(f"Calculating score for Team {team_num} at Location: {rakip_location}")
                        print(f"My Location: {my_lat_lon}, My Heading: {my_heading}")
    ...
    def thread_uav(self):
        ...
        if telemetry_raw:
            try:
                print(f"geliyoooooor")
                telemetry_str = json.loads(telemetry_raw)
                telemetry_data =  telemetry_str.get("konumBilgileri", {}).get("konumBilgileri", [])


                print(f"Initial telemetry data: {telemetry_data}")
        ...
        # 1) Kendi uçağınızı (team 1) listeden atıyoru
