...
mavlink_handler = MAVLinkHandler(f'{uav_ip}:{uav_port}')
team_number = 5
GOAL_NUMBER = config_data["tahmini_yogunluk_merkezi"]["x"] 

# Mevcut hedefi global olarak tut
current_goal = None

def get_redis_data():
    anlik_konum = r.get('anlik_konumlar')
    tahmini_yogunluk_merkezi = r.get('tahmini_yogunluk_merkezleri')
...
def generate_waypoints(smoothed_path, distance_interval=0.002): #200 metre aralıkla çiziyo
...
def run_waypoint_mission(start, goal, hss_data, bounds=None):
    ...
    while True:
        ...
        top_goals = tahmini_yogunluk_merkezi[:GOAL_NUMBER]
...
def find_valid_goal(tahmini_yogunluk_merkezleri, hss_data):
    for goal in tahmini_yogunluk_merkezleri:
        if not is_in_hss_area(goal, hss_data):
            return goal
    return None
...
def main():
    ...
    while True:
        ...
        if start and tahmini_yogunluk_merkezi:
            goal = find_valid_goal(tahmini_yogunluk_merkezi[:GOAL_NUMBER], hss_data)
            if goal:
                global current_goal
                current_goal = goal
                break
