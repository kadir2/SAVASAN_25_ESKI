...
is_local = config_data["uav_handler"]["is_local"]
if is_local:
    uav_ip = config_data["uav_handler"]["yerel_ip"]
else: 
    uav_ip = config_data["uav_handler"]["uav_ip"]
uav_port = config_data["uav_handler"]["uav_port_0"]
print(f'{uav_ip}:14551')  
mavlink_handler = MAVLinkHandler('127.0.0.1:14553')
team_number = 1
GOAL_NUMBER = 1
...
def get_redis_data():
    ...
    logging.info(f"Anlık konum: {anlik_konum}, Tahmini yoğunluk merkezi: {tahmini_yogunluk_merkezi}")
    return anlik_konum, tahmini_yogunluk_merkezi

def get_team_start_point(anlik_konum, team_number=team_number):
...
def generate_waypoints(smoothed_path, distance_interval=0.0002): #200 metre aralıkla çiziyo
...
def run_waypoint_mission(start, goal, hss_data, bounds=None):
    ...
    while True:
        ...
        # guid = int(r.get("guid"))
        guid = 2  # Simulate GUID message for testing

        print(f"GUID == {guid}")

        if guid != 2:
            print('guid message not 2 (cont loop)')
            time.sleep(0.3)
            continue
        else:
            logging.debug(f"GUID MASSAGE == {guid}, STAY AT GUIDED")
            if mavlink_handler.get_mode() != "GUIDED":
        ...
        top_goals = tahmini_yogunluk_merkezi
        new_goal = find_valid_goal(top_goals, hss_data)
        ...
        logging.info(f"Yol planlandı: {smoothed_path}")
        waypoints = generate_waypoints(smoothed_path, distance_interval=0.0002)  # 20m aralıklarla waypoint oluştur
        logging.info(f"Waypoint'ler oluşturuldu: {waypoints}")
        ...
        for index, (lat, lon) in enumerate(waypoints):
            ...
            while True:
                ...
                if guid != 2:
                    time.sleep(0.3)
                    print('guid message not 2 (break loop)')
                    break
                current_lat, current_lon, _ = mavlink_handler.get_location()
                distance = euclidean((current_lat, current_lon), (lat, lon))
                if distance < 0.001:  # yaklaşık 100m hata payı
        ...
        if euclidean((current_lat, current_lon), goal) < 0.001:
            logging.info("Ana hedefe ulaşıldı, yeni hedef bekleniyor.")
            time.sleep(0.5)

def is_in_hss_area(point, hss_data):
    logging.info(f"313131313{point} SSSSSSSSSSSSSSSSS")
    # lat = float(point['predicted_lat'])
    # lon = float(point['predicted_lon'])
    lat, lon = point
...
def find_valid_goal(tahmini_yogunluk_merkezleri, hss_data):
    if not is_in_hss_area(tahmini_yogunluk_merkezleri, hss_data):
        return tahmini_yogunluk_merkezleri

def main():
    hss_file_path = "hss_data.json"
    hss_data = load_hss_data(hss_file_path)

    # Uçuş sınırları (istiyorsan kullan, istemezsen None yap)
    bounds = (41.074955, 41.116336, 28.518404, 28.570011)

    # İlk başlangıç noktasını al
    while True:
        anlik_konum, tahmini_yogunluk_merkezi = get_redis_data()
        start = get_team_start_point(anlik_konum, team_number=team_number)
        logging.info(f"Başlangıç noktası alındı: {start}")
        logging.info(f"Tahmini yoğunluk merkezi: {tahmini_yogunluk_merkezi}")
        if start and tahmini_yogunluk_merkezi:
            goal = find_valid_goal(tahmini_yogunluk_merkezi, hss_data)
            if goal:
                global current_goal
                current_goal = goal
                break
    ...
    # Waypoint görevini başlat
    run_waypoint_mission(start, goal, hss_data, bounds)
